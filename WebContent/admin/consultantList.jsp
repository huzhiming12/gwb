...
<body>
	...
	<div class="container-fluid" style="min-height: 700px;">
		...
		<div class="row-fluid">
			...
			<div class="span9">
				<h1 class="page-title">顾问列表</h1>
				<hr>
				<form action="admin/cons_consultantList" id="search_form" method="post">
					<div class="btn-toolbar">
						<div class="input-append">
							<input class="span3" name="pager.condition['condition']"
								value="<s:property value="pager.condition['condition']"/>" placeholder="账号、姓名、职位"
								type="text">
							<button class="btn" id="btn_search" type="button">搜索</button>
							<button class="btn btn-success" id="btn_export" type="button">
				...
				<div class="well">
					<table class="table table-striped">
						...
						<tbody>
							<s:iterator value="consultants" id="c" status="cons">
								<tr>
									<td>
										<s:property value="#cons.index+1" />
									</td>
									<td>
										<a href="admin/cons_consultantInfo?consultant.id=<s:property value="#c.id" />">
											<s:property value="#c.mobilePhone" />
										</a>
									</td>
									<td>
									...
									<td>
										<s:if test="#c.state==0">
											<a href="#reviewModal" data-toggle="modal"
												onclick="reviewId = '<s:property value="#c.id"/>'" role="button" title="审核">
												<i class="icon-pencil"></i>
											</a>
										</s:if>
										<a href="#myModal" role="button" title="删除"
											onclick="delId = '<s:property value="#c.id"/>'" data-toggle="modal">
											<i class="icon-remove"></i>
										</a>
										<s:if test="#c.state==1">
											<a href="#lockModal" role="button" title="禁用该用户"
												onclick="lockId='<s:property value="#c.id"/>'" data-toggle="modal">
				...
				<div class="modal small hide fade" id="reviewModal" tabindex="-1" role="dialog"
					...
					<div class="modal-header">
						...
						<h3 id="myModalLabel">顾问审核</h3>
					</div>
					<div class="modal-body">
						<p class="error-text">
							<i class="icon-warning-sign modal-icon"></i> 是否同意该用户成为顾问帮顾问？
						</p>
					</div>
	...
	<script type="text/javascript">
		...
		$("#btn_export").click(function() {
			window.location.href = "admin/cons_exportConsList";
		});

		$("#btn_disagree").click(function() {
			$.post("admin/cons_reviewConsultant", {
				"consultant.id" : reviewId,
				"consultant.state" : 2
		...
		$("#btnLock").click(function() {
			$.post("admin/cons_lockConsultant", {
				"consultant.id" : lockId,
			}, function(data, status) {
				if (data == true) {
					alert("禁用成功！");
					location.reload();
		...
		$("#btnUnlock").click(function() {
			$.post("admin/cons_unlockConsultant", {
				"consultant.id" : lockId,
			}, function(data, status) {
				if (data == true) {
					alert("启用成功！");
